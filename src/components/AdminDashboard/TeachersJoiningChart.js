...
function TeachersJoiningChart(props) {
  ...
  async function getStudentsJoiningTrend() {
    setLoading(true);
    try {
      const { error, res } = await getRequest("teachers/joining-trend");
      if (error) {
        setErrorMsg(
          getErrorMsg(error, "Couldn't get the teachers joining trend")
        );
        setLoading(false);
        return;
      }
      ...
      setErrorMsg(getErrorMsg(err, "Couldn't get the teachers joining trend"));
    }
    setLoading(false);
  }
...
export default TeachersJoiningChart;
