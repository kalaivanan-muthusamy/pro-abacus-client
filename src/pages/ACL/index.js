...
function ACL(props) {
  ...
  function onCheckboxChange(key) {
    const currentState = examDetails[key];
    setExamDetails({
      ...examDetails,
      [key]: !currentState,
    });
  ...
  async function createACL(event) {
    try {
      ...
      if (error) {
        setLoading(false);
        setErrorMsg(getErrorMsg(error, "Couldn't schedule the ACL now"));
        errorRef?.current?.focus();
        return;
      }

      toastr.info("ACL has been scheduled successfully");
      history.push(`/dashboard`);
    } catch (err) {
      setErrorMsg("Couldn't schedule the ACL now");
      errorRef?.current?.focus();
    }
    setLoading(false);
  }
...
export default withNamespaces()(ACL);
